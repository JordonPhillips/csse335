...
void slave();
void MPI_matrix_vector_multiply(Matrix *result, Matrix *a, Matrix *b, int n,
...
void master(char *matrix_fname, char *vector_fname, char *out_fname) {
    ...
    MPI_matrix_vector_multiply(&result, &matrix, &vector, matrix.width, 0,
                               MPI_COMM_WORLD);
...
void MPI_matrix_vector_multiply(Matrix *result, Matrix *matrix, Matrix *vector,
    ...
    MPI_Scatter(rank == root ? matrix->data : NULL, a_size, MPI_FLOAT, A.data,
                a_size, MPI_FLOAT, root, comm);
    ...
    if (rank_i == root_row) {
        MPI_Scatter(rank == root ? vector->data : NULL, v_size, MPI_FLOAT, x.data,
                    v_size, MPI_FLOAT, root, row_comm);
    } else {
    ...
    if (rank_j == 0)
        MPI_Gather(x.data, v_size, MPI_FLOAT, rank == root ? result->data : NULL,
                   v_size, MPI_FLOAT, root, column_comm);
